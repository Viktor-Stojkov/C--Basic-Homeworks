using System;
using WorkShop04___ATM.Classes;

namespace WorkShop04___ATM
{
    class Program
    {
        static bool ChooseOption(Customer customer)
        {
            ...
            {
                case 1:
                    Console.WriteLine(string.Format("Your balance is: {0:C}", customer.ForBalance()));
                    break;
                case 2:
                    Console.WriteLine("How much would you like to withdraw?");
                    bool isAmountWNumber = int.TryParse(Console.ReadLine(), out int amount);
                    if (!isAmountWNumber)
                    {
                        Console.WriteLine("Input was not correct. The balance was not changed.");
                        break;
                    }
                    bool withdrawl = customer.MakeWithdraw(amount);
                    if (!withdrawl)
                    {
                    ...
                    {
                        Console.WriteLine(string.Format("You withdrawed {0:C}. You have {1:C} left on your account.", amount, customer.ForBalance()));
                    }
                    break;
                case 3:
                    Console.WriteLine("Enter amount of deposit:");
                    bool isAmountDNumber = int.TryParse(Console.ReadLine(), out int amountD);
                    if (!isAmountDNumber)
                    {
                        Console.WriteLine("Input was not correct. The balance was not changed.");
                        break;
                    ...
                    Console.WriteLine(string.Format("You deposited {0:C}. You have {1:C} on your account.", amountD, customer.ForBalance()));
                    break;
                case 4:
        ...
        static bool InitATM(Customer[] customers)
        {
            ...
            bool isPinNumber = short.TryParse(Console.ReadLine(), out short pin);
            if (!isPinNumber)
            {
